...
public class WorkWithLogin {
	...
	private By btnMenu = By.id("menu-toggle");
	private By btnLogout = By.cssSelector("a[href='authenticate.php?logout']");
    ...
    public void clickLogout(){
  		this.automator.waitUntilClickable(btnLogout);
  		logger.info("Click on the Logout button");
  		this.automator.find(btnLogout).click();
  	}
    public void clickMenu(){
  		this.automator.waitUntilClickable(btnMenu);
  		logger.info("Click on the menu button");
  		this.automator.find(btnMenu).click();
  	}
    ...
    public void logoutCURA(){  
        this.clickMenu();
        WebDriverWait wait = new WebDriverWait(this.automator.getDriver(), 30);
        this.clickLogout();
     }
	//Verify
